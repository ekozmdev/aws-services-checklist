...
| --- | --- | --- | --- | --- |
| Amazon EC2 | はい | はい | 業務で運用しており、基本機能に慣れている。AMIやインスタンスタイプで構成を選ぶ理解がある。[4] | 2026/01/24 |
...
| サービス名 | 知っている | 触ったことがある | メモ | 更新日 |
| --- | --- | --- | --- | --- |
| Amazon S3 | はい | はい | 運用としては深くないが、触った経験はある。バケット/オブジェクト/キーの関係と、バケットポリシー中心のアクセス制御を整理した。[4] 新規バケットは2023年からACLが無効化されるデフォルトだと理解した。[1] | 2026/01/24 |
| Amazon EFS | いいえ | いいえ |  |  |
| Amazon FSx | いいえ | いいえ |  |  |
...
| サービス名 | 知っている | 触ったことがある | メモ | 更新日 |
| --- | --- | --- | --- | --- |
| Amazon RDS | はい | いいえ | マネージドなリレーショナルDBで、OS運用なしに使える点を押さえた。冗長構成やリードレプリカも前提にあると理解した。[4] | 2026/01/24 |
...
| サービス名 | 知っている | 触ったことがある | メモ | 更新日 |
| --- | --- | --- | --- | --- |
| Amazon VPC | はい | いいえ | VPCで仮想ネットワークを作り、サブネットはAZ単位で分ける理解。NACLはサブネット単位のステートレス制御で、セキュリティグループは状態管理する前提だと把握した。[4] | 2026/01/24 |
...
| AWS Activate for Startups | - | - | スタートアップ向けの申請・審査が前提で、個人利用は難しい。 | 2026/01/24 |
...
| サービス名 | 知っている | 触ったことがある | メモ | 更新日 |
| --- | --- | --- | --- | --- |
| Amazon EBS | はい | いいえ | EC2向けのブロックストレージで、スナップショットはS3に保管されると把握した。[4] | 2026/01/24 |
| Amazon EC2 Auto Scaling | はい | いいえ | 起動テンプレートと最小/最大/希望数を設定し、ポリシーで台数を増減させる仕組みを学んだ。ELBと組み合わせて使う前提を理解した。[4] | 2026/01/24 |
...
| Elastic Load Balancing | はい | いいえ | ALBはレイヤー7、NLBはレイヤー4で用途が分かれると把握した。ELB自体はAZに冗長化され自動でスケールする前提と理解した。[4] | 2026/01/24 |
...
4. [AWS運用入門 改訂第2版 押さえておきたいAWSの基本と運用ノウハウ](https://www.sbcr.jp/product/4815631086/)
